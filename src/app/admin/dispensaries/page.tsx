...
export default function DispensariesPage() {
  ...
  const fetchDispensaries = useCallback(async () => {
    ...
    try {
      ...
      const res = await adminFetch(`/api/admin/dispensaries?${params}`);
      if (res.ok) {
        setDispensaries(res.data);
        setTotal(res.total);
        setPendingCount(res.pending_count);
      } else {
        setError(res.error || 'Failed to load dispensaries');
      }
    } catch {
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <p style={{ fontSize: '0.85rem', color: TEXT_SECONDARY }}>
            {total} dispensar{total === 1 ? 'y' : 'ies'} registered
          </p>
        </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.84rem' }}>
              <thead>
                <tr style={{ borderBottom: `1px solid ${BORDER}` }}>
                  {['Company Name', 'License #', 'License Type', 'Contact', 'Email', 'Status', 'Created'].map(
                    (header) => (
                      <th
                        key={header}
                        style={{
              ...
              <tbody>
                {loading ? (
                  ...
                  dispensaries.map((d) => (
                    <tr
                      key={d.id}
                      onClick={() => router.push(`/admin/dispensaries/${d.id}`)}
                      style={{
                    ...
                    >
                      ...
                      <td style={{ padding: '14px 16px', color: TEXT_SECONDARY }}>
                        {formatLicenseType(d.license_type)}
                      </td>
