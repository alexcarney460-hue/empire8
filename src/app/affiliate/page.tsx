...
export default function AffiliatePage() {
  return (
    <div style={{ paddingTop: 'var(--nav-height)', backgroundColor: '#0F0520', minHeight: '100vh' }}>
      ...
      {/* HERO */}
      <section style={{ backgroundColor: '#0F0520', borderBottom: '1px solid rgba(200,162,60,0.12)', padding: '72px 24px 80px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', bottom: '-10%', left: '3%', width: 300, height: 300, borderRadius: '50%', background: 'rgba(74,14,120,0.15)', filter: 'blur(60px)', pointerEvents: 'none' }} />
        <div style={{ maxWidth: 800, margin: '0 auto', textAlign: 'center', position: 'relative', zIndex: 1 }}>
          ...
          <h1 className="font-display" style={{ fontSize: 'clamp(2.5rem, 6vw, 4.5rem)', lineHeight: 1.0, color: '#fff', marginBottom: 20, letterSpacing: '-0.01em' }}>
            Become a Partner
          </h1>
          <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '1.1rem', maxWidth: 580, margin: '0 auto 40px', lineHeight: 1.8 }}>
      ...
      {/* WHY PARTNER */}
      <section style={{ backgroundColor: '#150A28', borderBottom: '1px solid rgba(200,162,60,0.12)', padding: '80px 24px' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          <AnimateIn style={{ marginBottom: 52 }}>
            <h2 className="font-display" style={{ fontSize: 'clamp(1.75rem, 4vw, 2.5rem)', color: '#fff', textAlign: 'center' }}>
            ...
            {[
              ...
              <AnimateIn key={title} delay={i * 70}>
                <div style={{ display: 'flex', gap: 14 }}>
                  ...
                  <div>
                    <h3 className="font-heading" style={{ fontSize: '0.95rem', marginBottom: 6, color: '#fff' }}>{title}</h3>
                    <p style={{ fontSize: '0.875rem', color: 'rgba(255,255,255,0.6)', lineHeight: 1.7 }}>{desc}</p>
                  </div>
                </div>
              </AnimateIn>
            ))}
      ...
      {/* COMMISSION TIERS */}
      <section style={{ backgroundColor: '#0F0520', borderBottom: '1px solid rgba(200,162,60,0.12)', padding: '80px 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <AnimateIn style={{ textAlign: 'center', marginBottom: 48 }}>
            <p style={{ color: 'rgba(255,255,255,0.6)', maxWidth: 560, margin: '0 auto', fontSize: '0.95rem', lineHeight: 1.7 }}>
              ...
              <AnimateIn key={name} delay={i * 70}>
                <div
                  ...
                  style={{
                    ...
                    borderRadius: 20,
                    padding: '28px 20px',
                    border: highlighted ? '2px solid var(--color-gold)' : '1px solid rgba(200,162,60,0.12)',
                ...
                >
                  ...
                  <h3 className="font-heading" style={{ fontSize: '1rem', marginBottom: 6, color: '#fff' }}>{name}</h3>
                  <p style={{ fontSize: '0.78rem', color: 'rgba(255,255,255,0.6)', marginBottom: 16 }}>{volume}</p>
                  <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {perks.map((perk) => (
                      <li key={perk} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.78rem', color: perk.includes('NET-7') ? 'var(--color-gold)' : 'rgba(255,255,255,0.6)', fontWeight: perk.includes('NET-7') ? 700 : 400 }}>
          ...
          <AnimateIn>
            <div style={{ marginTop: 24, padding: '20px 28px', backgroundColor: 'rgba(200,146,42,0.06)', border: '1px solid rgba(200,146,42,0.22)', borderRadius: 14 }}>
              <p style={{ fontSize: '0.875rem', color: '#fff', lineHeight: 1.7, margin: 0 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 12 }}>
            {WHO.map((item, i) => (
              <AnimateIn key={item} delay={i * 80}>
                ...
                >
                  <span style={{ color: 'var(--color-gold)', fontWeight: 800, fontSize: '1rem' }}>&rarr;</span>
                  <span className="font-heading" style={{ fontSize: '0.92rem', color: '#fff' }}>{item}</span>
                </div>
              </AnimateIn>
            ))}
          </div>
      ...
      {/* APPLICATION FORM */}
      <section id="apply" style={{ backgroundColor: '#150A28', borderBottom: '1px solid rgba(200,162,60,0.12)', padding: '80px 24px' }}>
        <div style={{ maxWidth: 760, margin: '0 auto' }}>
          <AnimateIn>
            <div style={{ backgroundColor: 'rgba(255,255,255,0.04)', border: '1px solid rgba(200,162,60,0.12)', borderRadius: 24, padding: '52px 44px', boxShadow: 'var(--shadow-sm)' }}>
              <h2 className="font-display" style={{ fontSize: 'clamp(1.5rem, 3vw, 2rem)', color: '#fff', marginBottom: 12 }}>
      ...
      <section style={{ backgroundColor: '#0F0520', borderBottom: '1px solid rgba(200,162,60,0.12)', padding: '80px 24px' }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          <AnimateIn style={{ marginBottom: 48 }}>
            <h2 className="font-display" style={{ fontSize: 'clamp(1.75rem, 4vw, 2.25rem)', color: '#fff', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
            {[
              ...
              <AnimateIn key={q}>
                <div>
                  <h3 className="font-heading" style={{ fontSize: '1rem', marginBottom: 8, color: '#fff' }}>{q}</h3>
                  {contact ? (
                    ...
                    <p style={{ fontSize: '0.9rem', color: 'rgba(255,255,255,0.6)', lineHeight: 1.75 }}>{a}</p>
                  )}
                </div>
              </AnimateIn>
            ))}
      ...
      {/* Compliance notice */}
      <section style={{ backgroundColor: '#150A28', padding: '40px 24px', borderBottom: '1px solid rgba(200,162,60,0.12)' }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
