...
const NAV_ITEMS: readonly NavItem[] = [
  { label: 'Overview', href: '/brand-dashboard', icon: LayoutDashboard },
  { label: 'My Products', href: '/brand-dashboard/products', icon: Package },
...
export default function BrandDashboardLayout({ children }: { children: ReactNode }) {
  ...
  // Fetch company name; redirect if not authenticated
  useEffect(() => {
    async function loadBrandInfo() {
      try {
        const res = await fetch('/api/brand-dashboard/stats');
        if (res.status === 401) {
