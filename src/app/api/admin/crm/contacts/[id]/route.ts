...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    console.error('[contacts-detail] Unexpected error:', err instanceof Error ? err.message : err);
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    console.error('[contacts-patch] Unexpected error:', err instanceof Error ? err.message : err);
    return NextResponse.json({ ok: false, error: 'An internal error occurred.' }, { status: 500 });
  }
}
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    console.error('[contacts-delete] Unexpected error:', err instanceof Error ? err.message : err);
    return NextResponse.json({ ok: false, error: 'An internal error occurred.' }, { status: 500 });
  }
}
