...
export async function GET(req: Request) {
  try {
    ...
    console.error('[contacts-list] Unexpected error:', err instanceof Error ? err.message : err);
    return NextResponse.json({ ok: false, error: 'An internal error occurred.' }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    console.error('[contacts-create] Unexpected error:', err instanceof Error ? err.message : err);
    return NextResponse.json({ ok: false, error: 'An internal error occurred.' }, { status: 500 });
  }
}
