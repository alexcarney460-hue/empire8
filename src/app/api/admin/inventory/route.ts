...
export async function GET(req: Request) {
  try {
    ...
    console.error('[inventory-list] Unexpected error:', err instanceof Error ? err.message : err);
    return NextResponse.json({ ok: false, error: 'An internal error occurred.' }, { status: 500 });
  }
}
...
export async function PATCH(req: Request) {
  try {
    ...
    console.error('[inventory-patch] Unexpected error:', err instanceof Error ? err.message : err);
    return NextResponse.json({ ok: false, error: 'An internal error occurred.' }, { status: 500 });
  }
}
